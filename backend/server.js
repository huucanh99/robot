...
// ─── vision_opencv.py keep-alive (Live Feed only — detect vẫn do TMflow đảm nhiệm) ────
// Chạy vision_opencv.py ở chế độ chỉ poll camera liên tục để Live Feed cập nhật real-time,
// KHÔNG gửi kết quả detect qua port 8765 (tránh ghi đè dữ liệu Pattern Matching của TMflow).
// Tắt bằng AUTO_START_VISION=0 trong .env nếu muốn tự chạy `python vision_opencv.py` tay
// (ví dụ để dùng cửa sổ debug "Detected" cho việc calibrate — xem SOP Part 5.1).
let visionProcess = null
let visionRestartCount = 0
const VISION_MAX_RESTARTS = 5
function startVisionKeepAlive() {
  const scriptPath = path.join(__dirname, "vision_opencv.py")
  visionProcess = spawn(process.env.PYTHON_BIN || "python", [scriptPath], {
    env: {
      ...process.env,
      VISION_PC_IP: "127.0.0.1",
      VISION_DEBUG: "0",
      VISION_SEND_DETECTIONS: "0",
    },
  })
  visionProcess.stdout.on("data", d => console.log(`[vision_opencv] ${d.toString().trim()}`))
  visionProcess.stderr.on("data", d => console.warn(`[vision_opencv][err] ${d.toString().trim()}`))
  visionProcess.on("error", e => console.warn("[vision_opencv] failed to start:", e.message))
  visionProcess.on("exit", (code, signal) => {
    visionProcess = null
    if (signal) return  // killed on purpose (shutdown) — don't restart
    visionRestartCount++
    if (visionRestartCount > VISION_MAX_RESTARTS) {
      console.warn(`[vision_opencv] failed ${visionRestartCount} times in a row — giving up. ` +
        `Live Feed will only update on Scan. Check PYTHON_BIN / python install, or set AUTO_START_VISION=0.`)
      return
    }
    const delay = Math.min(3000 * 2 ** (visionRestartCount - 1), 60000)
    console.warn(`[vision_opencv] exited (code ${code}) — restarting in ${delay / 1000}s... (attempt ${visionRestartCount}/${VISION_MAX_RESTARTS})`)
    setTimeout(startVisionKeepAlive, delay)
  })
}
if (process.env.AUTO_START_VISION !== "0") startVisionKeepAlive()

function stopVisionKeepAlive() {
  if (visionProcess) { visionProcess.removeAllListeners("exit"); visionProcess.kill() }
}
process.on("SIGINT",  () => { stopVisionKeepAlive(); process.exit(0) })
process.on("SIGTERM", () => { stopVisionKeepAlive(); process.exit(0) })
