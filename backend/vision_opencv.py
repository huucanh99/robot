...
YOUR_PC_IP  = os.environ.get("VISION_PC_IP", "169.254.243.1")  # IP máy của bạn (máy robot arm kết nối tới)
CAMERA_URL  = f"http://{YOUR_PC_IP}:3000/camera/capture"
VISION_HOST = YOUR_PC_IP             # gửi kết quả về máy của bạn
VISION_PORT = 8765
FPS_DELAY   = 0.1   # 10fps
DEBUG       = os.environ.get("VISION_DEBUG", "1") == "1"              # tắt khi chạy ngầm (không có màn hình)
SEND_DETECTIONS = os.environ.get("VISION_SEND_DETECTIONS", "1") == "1"  # tắt khi chỉ cần ảnh live (TMflow lo phần detect)
...
print("[OpenCV Vision] Starting..." if SEND_DETECTIONS else
      "[OpenCV Vision] Starting in keep-alive mode (no detection sent — TMflow is the detection source)...")
last_found = None
miss_count = 0
...
while True:
    try:
        if not SEND_DETECTIONS:
            # Keep-alive mode: chỉ poll camera để /camera/capture liên tục cập nhật
            # lastCapturedImage cho Live Feed — không đụng tới detect (TMflow lo phần đó).
            get_frame()
        else:
            frame = get_frame()
